...
algorithm_regtest() {   #1st arg: deployment_config file  ; #2nd: application_config file
    ...
    algo=`grep "defaultalgorithm" ${app_file} | cut -d: -f2` 
    filesize=$(( `grep "default_file_size" ${app_file} | cut -d: -f2` ))
    num_servers_per_object=$(( `grep "num_servers_per_object" ${app_file} | cut -d: -f2` ))

    outputfile=${algo}'_'${numwriters}'W_'${numreaders}'R_'${numservers}'S_'${filesize}'FS_'${num_servers_per_object}'SPO'
    ofolder="../../results"
    ...
    echo analytics/expAnalysis/expAnalyzer -logroot="/tmp/ercost/" -results=$outputfile -deployyml=${dep_file} -appyml=${app_file} -outputfolder=${ofolder}
    analytics/expAnalysis/expAnalyzer -logroot="/tmp/ercost/" -results=$outputfile -deployyml=${dep_file} -appyml=${app_file} -outputfolder=${ofolder}

    echo "================= " >> ../../results/$outputfile
    echo "Safety Check Results Below" >> ../../results/$outputfile

    mkdir /tmp/ercost/verify
    cat  /tmp/ercost/*er-*/logs/applog.txt  | grep FOR_SAFETY_CHECK | sed -e 's/FOR_SAFETY_CHECK:\s*//g'   >  /tmp/ercost/verify/safety.log
    echo analytics/safetyAnalyzer -safety_log_path /tmp/ercost/verify/safety.log -numreaders ${numreaders} -numwriters ${numwriters} -numreads ${numreads} -numwrites ${numtotwrites}
    analytics/safetyAnalyzer -safety_log_path /tmp/ercost/verify/safety.log -numreaders ${numreaders} -numwriters ${numwriters} -numreads ${numreads} -numwrites ${numtotwrites} >> ${ofolder}/${outputfile}
